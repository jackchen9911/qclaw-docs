---
title: "支持的文字模型"
description: "文字对话模型一览，支持 Chat Completions 与 Responses 两种 API 格式"
---
...
| API 格式 | 端点 | 说明 |
|----------|------|------|
| **Chat Completions**（默认） | `POST /v1/chat/completions` | 经典对话接口，兼容所有 OpenAI SDK |
| **Responses**（新） | `POST /v1/responses` | OpenAI 新一代 Agentic API，支持函数调用、内置工具、服务端多轮上下文 |

<Note>
  标注 `Responses Only` 的模型仅支持 Responses API，不支持 Chat Completions。其余模型默认通过 Chat Completions 调用。
</Note>
...
| `gpt-5.3-codex-official` | GPT-5.3 Codex 官方直连，最新代码优化版 | ✓ | ✓ | Responses Only |
...
| `gemini-3.1-flash-lite-preview-official` | Gemini 3.1 Flash Lite，最具成本效益的轻量模型 | ✓ |  |
| `gemini-3-pro-official` | Gemini 3 Pro 旗舰，最强推理与多模态能力 | ✓ | ✓ |
| `gemini-3-pro-preview-official` | Gemini 3 Pro Preview 版本 | ✓ | ✓ |
| `gemini-3-flash-official` | Gemini 3 Flash，快速高效，性价比之选 | ✓ | ✓ |
| `gemini-3-flash-preview-official` | Gemini 3 Flash Preview 版本 | ✓ | ✓ |
